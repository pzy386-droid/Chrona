...
Rectangle {
    ...
    signal moveRequested(int blockId,
                         int dayIndex,
                         int startMinute,
                         int durationMinutes)
    ...
    Component.onCompleted: {
        root.readyForResizeAnimation = true
    }
    ...
    MouseArea {
        ...
        drag.maximumX: root.timelineLeft
                      + Math.max(0, root.dayCount - 1)
                      * root.dayWidth
                      + root.horizontalInset
        ...
        onPressed: {
            ...
            ScheduleService.selectTimelineItem(
                        root.taskId,
                        root.blockId)
        ...
        onPositionChanged: {
            if (pressed
                    && (Math.abs(root.x - root.pressX) > 3
                        || Math.abs(root.y - root.pressY) > 3)) {
        ...
        onReleased: {
            ...
            root.x =
                    root.timelineLeft
                    + nextDay * root.dayWidth
                    + root.horizontalInset

            root.y =
                    (nextMinute - root.dayStartMinute)
                    * root.minuteHeight
    ...
    layer.enabled: mouse.containsMouse
